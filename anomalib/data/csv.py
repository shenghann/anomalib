...
Expected columns:
    full_path: Full absolute path to image file
"""
...
def _prepare_files_labels(
    ...
    Args:
        path (Union[str, Path]): Path to the CSV
    ...
    if 'full_path' not in csv_data:
        raise RuntimeError(f"Invalid CSV (missing required columns) file {path}") 
    
    filenames = [file for file in csv_data.full_path.tolist() if Path(file).suffix in extensions]
...
def _prepare_csv_labels(
    root: Union[str, Path], path: Union[str, Path], csv_type: str, extensions: Optional[Tuple[str, ...]] = None
) -> Tuple[list, list]:
    path = _check_and_convert_path(path)
    if extensions is None:
        extensions = IMG_EXTENSIONS
    ...
    # handle windows path
    csv_data['path'] = csv_data.apply(lambda row: root / PureWindowsPath(row.path).as_posix(), axis=1)

    if csv_type == 'train':
        csv_data['label'] = 'normal'
    elif csv_type == 'val':
        csv_data['label'] = csv_data.installation.apply(lambda label: 'normal_test' if label == 1 else 'abnormal')
    else:
...
def make_dataset(
    ...
    # what we need:
    # dataframe of normal (good) and abnormal (bad) images and normal_test (val_bad)
    csvs = {'train': train_csv, 'val': val_csv}

    for csv_type, path in csvs.items():
        filename, label = _prepare_csv_labels(root, path, csv_type, extensions)
        filenames += filename
        labels += label
...
@DATAMODULE_REGISTRY
class CSV(LightningDataModule):
    """CSV Lightning Data Module."""
